...
internal class DynamicResourceIdManager {
  private class IdProvider(private val packageByte: Byte) {
    ...
    fun getNext(type: ResourceType): Int {
      return app.cash.paparazzi.snapshot.internal.buildResourceId(
        packageByte,
        (type.ordinal + 1).toByte(),
        --counters[type.ordinal]
      )
    }
  }

  private var nextPackageId = app.cash.paparazzi.snapshot.internal.FIRST_PACKAGE_ID

  private val perNamespaceProviders = hashMapOf<ResourceNamespace, app.cash.paparazzi.snapshot.internal.DynamicResourceIdManager.IdProvider>()
  private val dynamicToIdMap = hashMapOf<ResourceReference, Int>()
  private val dynamicFromIdMap = hashMapOf<Int, ResourceReference>()

  init {
    perNamespaceProviders[ResourceNamespace.RES_AUTO] =
      app.cash.paparazzi.snapshot.internal.DynamicResourceIdManager.IdProvider(0x7f)
    perNamespaceProviders[ResourceNamespace.ANDROID] =
      app.cash.paparazzi.snapshot.internal.DynamicResourceIdManager.IdProvider(0x01)
  }
  ...
  fun getOrGenerateId(resource: ResourceReference): Int {
    ...
    val provider = perNamespaceProviders.getOrPut(resource.namespace) {
      app.cash.paparazzi.snapshot.internal.DynamicResourceIdManager.IdProvider(nextPackageId++)
    }
