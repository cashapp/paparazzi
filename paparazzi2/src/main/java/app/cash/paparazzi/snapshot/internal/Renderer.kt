...
import app.cash.paparazzi.snapshot.Device
import app.cash.paparazzi.snapshot.DeviceConfig
import app.cash.paparazzi.snapshot.Environment
import app.cash.paparazzi.snapshot.Flags
import app.cash.paparazzi.snapshot.getFieldReflectively
import app.cash.paparazzi.snapshot.internal.resources.AarSourceResourceRepository
import app.cash.paparazzi.snapshot.internal.resources.AppResourceRepository
import app.cash.paparazzi.snapshot.internal.resources.FrameworkResourceRepository
import app.cash.paparazzi.snapshot.setStaticValue
...
internal class Renderer(
    ...
    val (frameworkResources, projectResources) =
      if (useLegacyResourceLoading) {
        ResourceRepositoryBridge.Legacy(
          app.cash.paparazzi.snapshot.android.ide.common.resources.deprecated.FrameworkResources(
            app.cash.paparazzi.snapshot.android.io.FolderWrapper(
              platformDataResDir
            )
          )
            .apply {
          ...
          ResourceRepositoryBridge.Legacy(
            object : app.cash.paparazzi.snapshot.android.ide.common.resources.deprecated.ResourceRepository(
              app.cash.paparazzi.snapshot.android.io.FolderWrapper(environment.resDir), false
            ) {
              override fun createResourceItem(name: String): app.cash.paparazzi.snapshot.android.ide.common.resources.deprecated.ResourceItem {
                return app.cash.paparazzi.snapshot.android.ide.common.resources.deprecated.ResourceItem(
                  name
                )
              }
            }.apply { loadResources() }
  ...
  private fun configureBuildProperties() {
    // TODO: Figure out best class loader to use
    val classLoader = Device::class.java.classLoader
    val buildClass = try {
