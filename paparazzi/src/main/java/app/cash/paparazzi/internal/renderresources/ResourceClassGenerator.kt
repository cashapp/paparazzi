...
public class ResourceClassGenerator private constructor(
  ...
  private var styleableCache: MutableMap<String, List<Int>>? = null
    ...
    if (index != -1) {
      ...
      for (t in resources.getResourceTypes(namespace)) {
        if (t.hasInnerClass) {
          ...
          // https://youtrack.jetbrains.com/issue/KT-45856/Strange-resolution-of-name-property-in-enum-with-Java-interop
          cw.visitInnerClass(
  ...
  private fun generateStyleable(cw: ClassWriter, className: String) {
    ...
    for (mergedStyleable in mergedStyleables) {
      ...
      val values = mergedStyleable.attrs.map(idProvider::getOrGenerateId)
  ...
  private fun generateIntArraysFromCache(cw: ClassWriter, className: String) {
    ...
    val mv = cw.visitMethod(ACC_STATIC, "<clinit>", "()V", null, null)
    mv.visitCode()
    styleableCache!!.forEach { (arrayName: String, values: List<Int>) ->
  ...
  public companion object {
    private val LOG = Logger.getLogger(ResourceClassGenerator::class.java.name)
    ...
    private fun getStyleableAttributes(item: ResourceItem): List<ResourceReference> {
      ...
      return dv.allAttributes.map(AttrResourceValue::asReference)
    }
    ...
    private fun generateArrayInitialization(
      mv: MethodVisitor, className: String, fieldName: String, values: List<Int>
    ) {
      if (values.isEmpty()) {
        return
      }
    ...
    private fun StringBuilder.appendEscaped(v: String) {
