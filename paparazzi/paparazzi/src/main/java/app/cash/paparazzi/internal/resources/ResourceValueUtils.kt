...
fun parseResourceValue(
  ...
  return if (tag == null || type == PUBLIC) {
    val density =
      if (type == ResourceType.DRAWABLE || type == ResourceType.MIPMAP) repositoryConfiguration.folderConfiguration.densityQualifier?.value else null
    val path = file.absolutePath
    if (density != null) {
      DensityBasedResourceValueImpl(
        repositoryConfiguration.repository.namespace, type, name, path, density, null
      )
    } else {
      ResourceValueImpl(repositoryConfiguration.repository.namespace, type, name, path, null)
    }
  } else {
    parseXmlToResourceValueSafe(repositoryConfiguration.repository.namespace, type, name, tag)
  }
}

private fun parseXmlToResourceValueSafe(
  namespace: ResourceNamespace, type: ResourceType, name: String, tag: Element
): ResourceValue {
  val value: ResourceValueImpl = when (type) {
    ResourceType.STYLE -> {
      val parent = getAttributeValue(tag, SdkConstants.ATTR_PARENT).takeIf { it.isNotBlank() }
      parseStyleValue(tag, StyleResourceValueImpl(namespace, name, parent, null))
    }

    ResourceType.STYLEABLE -> {
      parseDeclareStyleable(
        namespace, tag, StyleableResourceValueImpl(namespace, name, null, null)
      )
    }
    ...
    ResourceType.ARRAY -> parseArrayValue(
      tag, ArrayResourceValueImpl(namespace, name, null)
    )

    ResourceType.PLURALS -> parsePluralsValue(tag,
      object : PluralsResourceValueImpl(namespace, name, null, null) {
        ...
        override fun getValue(): String {
          val quantity: String? =
            ...
            tag.getAttributeNS(
              SdkConstants.ATTR_QUANTITY, SdkConstants.TOOLS_URI
            )
          if (quantity != null) {
    ...
    ResourceType.STRING -> parseTextValue(
      tag,
      TextResourceValueImpl(namespace, name, null, null, null)
    )
...
private fun parseStyleValue(
  tag: Element, styleValue: StyleResourceValueImpl
): StyleResourceValueImpl {
  for (child in XmlUtils.getSubTags(tag)) {
    val name = getAttributeValue(child, SdkConstants.ATTR_NAME)
    if (name.isNotEmpty()) {
      val value = ValueXmlHelper.unescapeResourceString(getTextContent(child), true, true)
      val itemValue =
...
private fun parseDeclareStyleable(
  namespace: ResourceNamespace, tag: Element, declareStyleable: StyleableResourceValueImpl
...
private fun parseArrayValue(
  tag: Element, arrayValue: ArrayResourceValueImpl
): ArrayResourceValueImpl {
  for (child in XmlUtils.getSubTags(tag)) {
    val text = ValueXmlHelper.unescapeResourceString(getTextContent(child), true, true)
...
private fun parsePluralsValue(
  tag: Element, value: PluralsResourceValueImpl
): PluralsResourceValueImpl {
  for (child in XmlUtils.getSubTags(tag)) {
    val quantity: String? = child.getAttribute(SdkConstants.ATTR_QUANTITY)
    if (quantity != null) {
      val text = ValueXmlHelper.unescapeResourceString(getTextContent(child), true, true)
...
private fun parseTextValue(
  tag: Element, value: TextResourceValueImpl
...
private fun parseValue(
  tag: Element, value: ResourceValueImpl
): ResourceValueImpl {
  var text: String? = getTextContent(tag)
  text = ValueXmlHelper.unescapeResourceString(text, true, true)
  value.value = text
...
private fun parseAttrValue(
  attrTag: Element, attrValue: AttrResourceValueImpl
