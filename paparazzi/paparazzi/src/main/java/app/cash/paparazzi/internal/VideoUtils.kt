  ...
  fun assertVideoSimilar(
    ...
    try {
          ...
          if (encoder == null) {
            ...
            encoder = AWTSequenceEncoder.createSequenceEncoder(output, fps).also {
              ...
              for (i in 0 until index) {
                ...
                val resultDiff = calculateDifference(goldenImage2, testImage2)
                it.encodeImage(resultDiff.deltaImage)
              }
      ...
      val error = buildString {
        if (badFrameCount > 0) {
          appendLine(
            "$badFrameCount video frames differed by more than %.1f%%".format(maxPercentDifferent)
          )
        }
        if (!sameDuration) {
          appendLine("Mismatched video duration expected: ${goldenMeta.totalDuration} actual: ${testMeta.totalDuration}")
        }
        if (!sameFrameCount) {
          appendLine("Mismatched frame count expected: ${goldenMeta.totalFrames} actual: ${testMeta.totalFrames}")
        }
      }
