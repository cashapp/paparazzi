...
class PaparazziPluginTest {
  ...
  //
  //   gradleRunner
  //     .withArguments("testDebug", "--stacktrace", "--rerun")
  //     .runFixture(fixtureRoot) { build() }
  //
  //   val snapshotsDir = File(fixtureRoot, "build/reports/paparazzi/images")
  ...
  @Test
  fun customFontAndDrawables() {
    val fixtureRoot = File("src/test/projects/custom-fonts-multi-module")
    val moduleRoot = File(fixtureRoot, "fonts-view-module")

    gradleRunner
      .withArguments("fonts-view-module:testDebug", "--stacktrace")
      .runFixture(fixtureRoot) { build() }
    ...
    val snapshotImage = snapshots[0]
    val goldenImage = File(moduleRoot, "src/test/resources/app_compat_views.png")
    assertThat(snapshotImage).isSimilarTo(goldenImage).withDefaultThreshold()
  }
